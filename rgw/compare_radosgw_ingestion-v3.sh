...
## In this script, the difference is for that it will print Output that will show the top $NUM_TOP_DELTA_BUCKETS buckets with the highest delta increase. And v2 script does not print any header which was missing and added in this version


# --- Configuration ---
# Default threshold for objects per shard filter
DEFAULT_THRESHOLD=20000 # Your example had 20000, not 200000, please verify this value
...
# --- Generate Header and Process Data Separately, Then Combine ---

# 1. Store the header line in a variable (using tabs for column -t compatibility)
HEADER_LINE=$(printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
               "Bucket" "Shards" "Objects_Old" "Objects/Shard_Old" "Objects_Current" "Objects/Shard_Current" "Delta_Objects")

# 2. Process data with AWK, then sort and head the data *only*
#    The AWK script will NOT print the header.
DATA_LINES=$(awk '
BEGIN {
...
# Phase 2: Process the data from the NEW file (second input to awk)
{
    ...
    # Print only data lines (no header from AWK)
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n", \
           ...
           ops_current, \
           delta_objects
}' "$TEMP_OLD_DATA" "$TEMP_NEW_DATA" | sort -k7 -nr | head -n "$NUM_TOP_DELTA_BUCKETS")

# 3. Print the header, then the sorted data, and finally pipe the combined output to column -t
(echo "$HEADER_LINE"; echo "$DATA_LINES") | column -t
